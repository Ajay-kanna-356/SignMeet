...
class CameraController {
  ...
  onResults(results) {
    ...
    if (results.leftHandLandmarks) {
      drawing_utilsExports.drawConnectors(this.ctx, results.leftHandLandmarks, holisticExports.HAND_CONNECTIONS, { color: "#CC0000", lineWidth: 2 });
      drawing_utilsExports.drawLandmarks(this.ctx, results.leftHandLandmarks, { color: "#00FF00", lineWidth: 1 });
    }
    if (results.rightHandLandmarks) {
      drawing_utilsExports.drawConnectors(this.ctx, results.rightHandLandmarks, holisticExports.HAND_CONNECTIONS, { color: "#00CC00", lineWidth: 2 });
      drawing_utilsExports.drawLandmarks(this.ctx, results.rightHandLandmarks, { color: "#FF0000", lineWidth: 1 });
    }
    this.ctx.restore();
    const now = Date.now();
  ...
  async sendKeypoints(keypoints) {
    ...
    try {
      const storageResult = await new Promise((resolve) => {
        chrome.storage.local.get(["signmeet_voice_pref", "signmeet_lang_pref"], (result) => resolve(result));
      });
